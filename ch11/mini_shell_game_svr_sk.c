...
void* attack_thread(void* arg) {
    // 위 설명에 따라 코드를 작성하세요. 
    // your code here
    attack_arg_t *attack_arg = (attack_arg_t *)arg;
    for (int i = 0; i < attack_arg->damage / 0.01; i++) {
        ...
        if (ai_mode) {
            ...
            if (strcmp(argv[0], "attack") == 0) {
                ...
                for (int i = 1; argv[i] != NULL && i < MAXARGS; i++) {
                    ...
                    attack_arg_t attack_arg = { .damage = damage, .thread_id = i };

                    // 스레드 생성
                    pthread_create(&threads[i - 1], NULL, attack_thread, &attack_arg);
                }
