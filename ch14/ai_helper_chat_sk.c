...
int main(void) {
    ...
    // your code here
    void *context_mem;
    if ((context_mem = mmap(0, 1024 * 1024, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0)) == MAP_FAILED) {
        // 만약 매핑 실패 시 처리
        // your code here
        err_sys("mmap error");
    }
    ...
    const char *input1 = "제 이름은 이지섭이고 현재 시스템프로그래밍 수업을 듣고 있습니다.";
    printf("User: %s\n", input1);
    ...
    // 테스트 3
    printf("=== Test 3 ===\n");
    const char *input3 = "저는 현재 어떤 수업을 듣고 있죠?";
    printf("User: %s\n", input3);
